        ...
        <table className="w-full hidden md:table">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              ...
              <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Subcategoría de</th>
              <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Acciones</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {categorias?.length === 0 && (
              <tr><td colSpan={4} className="px-6 py-10 text-center text-gray-400">No hay categorías todavía. ¡Creá la primera!</td></tr>
            )}
            {categorias?.map(cat => (
              <tr key={cat.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 text-gray-500 text-sm">{cat.parent_id ? `#${cat.parent_id}` : '-'}</td>
          ...
          {categorias?.map(cat => (
            <div key={cat.id} className="p-4 flex justify-between items-center">
              <div>
                ...
                {cat.parent_id && <p className="text-xs text-gray-400">Sub de #{cat.parent_id}</p>}
              </div>
